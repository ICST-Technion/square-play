...
class Ai_player:
    ...
    def best_move_minmax_aux(self,board, players_pieces,turns_left_for_current_player,current_player):
        ...
        if turns_left_for_current_player > 0: #simulate the turn of a player
                ...
                for permutation in range(1,9):
                    for game_cord_x in range(0,32):
                        for game_cord_y in range(0, 32):
                            ...
                            if new_board.add_piece(current_player,piece,permutation,(game_cord_x,game_cord_y)) and new_board.new_squares() > 0:
                                new_player_piece = copy.deepcopy(players_pieces)
                                new_player_piece[current_player].remove(piece)
                                turn_options.append((self.best_move_minmax_aux(new_board,new_player_piece,turns_left_for_current_player-1+new_board.new_squares()-1,current_player)[0],
            ...
            return (self.best_move_minmax_aux(board,players_pieces,1,(current_player+1)%len(players_pieces))[0],None)
    ...
    def best_move_minmax(self,board, players_pieces, current_player_idx):
        ...
        return self.best_move_minmax_aux(board, players_pieces, 1, 0)[1] # returns (piece,permutation,game_cord_x,game_cord_y)
    ...
    def best_move_depth_minmax_aux(self,board, players_pieces,turns_left_for_current_player,current_player,depth):
        ...
        if turns_left_for_current_player > 0: #simulate the turn of a player
                    ...
                    for game_cord_x in range(0,32):
                        for game_cord_y in range(0, 32):
                            ...
                            if new_board.add_piece(current_player,piece,permutation,(game_cord_x,game_cord_y)) and new_board.new_squares() > 0:
                                new_player_piece = copy.deepcopy(players_pieces)
                                new_player_piece[current_player].remove(piece)
                                turn_options.append((self.best_move_depth_minmax_aux(new_board,new_player_piece,turns_left_for_current_player-1+new_board.new_squares()-1,current_player,depth)[0],
                                                     (piece,permutation,game_cord_x,game_cord_y)))
            if current_player == 0:
                return turn_options[self.argmax([a[0] for a in turn_options])]
            else:
            ...
            return (self.best_move_depth_minmax_aux(board,players_pieces,1,(current_player+1)%len(players_pieces),depth-1)[0],None)

    def best_move_depth_minmax(self,board, players_pieces,depth,current_player_idx): #depth is max number of turnes to predict
        for i in range(current_player_idx):
            players_pieces.append(players_pieces.pop(0))
        return self.best_move_depth_minmax_aux(board, players_pieces, 1, 0,depth)[0] # returns (piece,permutation,game_cord_x,game_cord_y)

    def best_move_depth_alphabeta_aux(self,alpha,beta,board, players_pieces,turns_left_for_current_player,current_player,depth,prob):
        #current player -> 0 is the caller, others are the other players.
        if len(players_pieces[current_player]) == 0:
            return (1,None) if current_player == 0 else (-1,None) #TODO check- should we consider last lap of game?
        if depth == 0:
        ...
        if turns_left_for_current_player > 0: #simulate the turn of a player
                        ...
                        for game_cord_y in range(0, 32):
                            if random.random()<prob:
                                ...
                                if new_board.add_piece(current_player,piece,permutation,(game_cord_x,game_cord_y)) and new_board.new_squares() > 0:
                                    new_player_piece = copy.deepcopy(players_pieces)
                                    new_player_piece[current_player].remove(piece)
                                    turn_options.append((new_board,new_player_piece,piece,permutation,game_cord_x,game_cord_y))
            if current_player == 0:
                best_val = -inf
                move = None
                for move_option in turn_options:
                    value = self.best_move_depth_alphabeta_aux(alpha,beta,move_option[0],move_option[1],turns_left_for_current_player-1+move_option[0].new_squares()-1,current_player,depth,prob)[0]
            ...
            return (self.best_move_depth_alphabeta_aux(alpha,beta,board,players_pieces,1,(current_player+1)%len(players_pieces),depth-1,prob)[0],None)

    def best_move_depth_alphabeta(self,board, players_pieces,depth,current_player_idx,prob=1): #depth is max number of turnes to predict, prob is probaillity to choose a child
        for i in range(current_player_idx):
            players_pieces.append(players_pieces.pop(0))
        return self.best_move_depth_alphabeta_aux(-inf,inf,board, players_pieces, 1, 0,depth,prob)[1] # returns (piece,permutation,game_cord_x,game_cord_y)


    def best_move_depth_alphabeta_timeLimit_aux(self, alpha, beta, board, players_pieces, turns_left_for_current_player, current_player, depth, prob,
        ...
        if turns_left_for_current_player > 0: #simulate the turn of a player
                ...
                for permutation in self.shape_permutation[piece]:
                    for game_cord_x in range(0,32):
                        for game_cord_y in range(0, 32):
                            if random.random()<prob:
                                new_board = copy.deepcopy(board)
                                if new_board.add_piece(current_player,piece,permutation,(game_cord_x,game_cord_y)) and new_board.count_new_squares() > 0:
                                    new_player_piece = copy.deepcopy(players_pieces)
                                    new_player_piece[current_player].remove(piece)
                                    turn_options.append((new_board,new_player_piece,piece,permutation,game_cord_x,game_cord_y))
            if current_player == 0:
                ...
                move = None
                for move_option in turn_options:
                    value = self.best_move_depth_alphabeta_timeLimit_aux(alpha,beta,move_option[0],move_option[1],turns_left_for_current_player-1+move_option[0].count_new_squares()-1,current_player,depth,prob,start_time, time_limit)[0]
                    if value == "time is up":
                        return "time is up", "time is up"
                    best_val = max(value,best_val)
                    if best_val == value:
                ...
                for move_option in turn_options:
                    value = self.best_move_depth_alphabeta_timeLimit_aux(alpha, beta, move_option[0], move_option[1],
                                                               turns_left_for_current_player - 1 + move_option[
                                                                   0].count_new_squares() - 1, current_player, depth,prob,start_time, time_limit)[0]
                    if value == "time is up":
                        return "time is up", "time is up"
                    best_val = min(value, best_val)
                    if best_val == value:
            ...
            return (self.best_move_depth_alphabeta_timeLimit_aux(alpha,beta,board,players_pieces,1,(current_player+1)%len(players_pieces),depth-1,prob,start_time, time_limit)[0],None)

    def best_move_depth_timeLimit_alphabeta(self, board, players_pieces, current_player_idx,
                                  prob=1,time_limit=300):  #prob is probaillity to choose a child (0.05 is fine)
        for i in range(current_player_idx):
            players_pieces.append(players_pieces.pop(0))

        start = time.time()
        depth = 1
        move = self.best_move_depth_alphabeta_timeLimit_aux(-inf, inf, board, players_pieces, 1, 0, depth, prob,start,inf)[1]
        while True:
            depth += 1
            res = self.best_move_depth_alphabeta_timeLimit_aux(-inf, inf, board, players_pieces, 1, 0, depth, prob,start,time_limit)[1]
            if res == "time is up":
    ...
    def score_of_state(self,board,players_pieces,player,moves_left): #if player is 1, then ai(0) played latest, so we calculate his state
        ...
        for line in board_line_list:
            ...
            if line_pos[0][1] == line_pos[1][1]: #line on x axis
                if {(line_pos[0][0],line_pos[0][1]),(line_pos[0][0],line_pos[0][1] + 1)} in board_line_list and (
                        {(line_pos[1][0],line_pos[1][1]),(line_pos[1][0],line_pos[1][1] + 1)} in board_line_list and not(
                        {(line_pos[0][0], line_pos[0][1] + 1), (line_pos[1][0], line_pos[1][1] + 1)} in board_line_list)):
                    count += 1
                if ({(line_pos[0][0], line_pos[0][1]),(line_pos[0][0], line_pos[0][1] - 1)} in board_line_list and (
                        {(line_pos[1][0], line_pos[1][1]),(line_pos[1][0], line_pos[1][1] - 1)} in board_line_list and not (
                        {(line_pos[0][0], line_pos[0][1] - 1),(line_pos[1][0], line_pos[1][1] - 1)} in board_line_list))):
            ...
            if line_pos[0][0] == line_pos[1][0]: #line on y axis
                if {(line_pos[0][0],line_pos[0][1]),(line_pos[0][0] + 1,line_pos[0][1])} in board_line_list and (
                        {(line_pos[1][0],line_pos[1][1]),(line_pos[1][0] + 1,line_pos[1][1])} in board_line_list and not(
                        {(line_pos[0][0] + 1, line_pos[0][1]), (line_pos[1][0] + 1, line_pos[1][1])} in board_line_list)):
                    count += 1
                if({(line_pos[0][0], line_pos[0][1]),(line_pos[0][0] - 1, line_pos[0][1])} in board_line_list and (
                        {(line_pos[1][0], line_pos[1][1]),(line_pos[1][0] - 1, line_pos[1][1])} in board_line_list and not(
                        {(line_pos[0][0] - 1, line_pos[0][1]), (line_pos[1][0] - 1, line_pos[1][1])} in board_line_list))):
    ...
    def best_move_depth_alphabeta_timeLimit_all_turn_aux(self, alpha, beta, board, players_pieces, turns_left_for_current_player,
        ...
        if time.time() - start_time > time_limit:
            return "time is up", "time is up"
        if len(players_pieces[current_player]) == 0:
            return (10, None) if current_player == 0 else (-10, None)  # TODO check- should we consider last lap of game?
        if depth == 0:
            return self.score_of_state(board, players_pieces, current_player, turns_left_for_current_player), None
        turn_options = []
        if turns_left_for_current_player > 0:  # simulate the turn of a player
           ...
           # print("done in:"+str(time.time() - start_time)+ "seconds\ncurrent player:"+str(current_player))###########
            ...
            if current_player == 0:
                ...
                move = None
                for move_option in turn_options:
                    value,move_ = self.best_move_depth_alphabeta_timeLimit_all_turn_aux(alpha, beta, move_option[0], move_option[1],
                ...
                for move_option in turn_options:
                    value, move_ = self.best_move_depth_alphabeta_timeLimit_all_turn_aux(alpha, beta, move_option[0], move_option[1],
                                                                         ...
                                                                         depth, prob, start_time, time_limit)
                    if value == "time is up":
                        return "time is up", move#"time is up"
                    best_val = min(value, best_val)
                    if best_val == value:
    ...
    def best_move_depth_alphabeta_timeLimit_all_turn(self, board, players_pieces, current_player_idx,
        ...
        start = time.time()
        depth = 1
        ret_val, move = self.best_move_depth_alphabeta_timeLimit_all_turn_aux(-inf, inf, board, players_pieces, 1, 0, depth, prob,start,time_limit)#not inf in time limit
        ...
        while True:
            depth += 1
            res = self.best_move_depth_alphabeta_timeLimit_all_turn_aux(-inf, inf, board, players_pieces, 1, 0, depth, prob,start,time_limit)[1]
    ...
    def calc_move(self, board, players_pieces, current_player_idx):
        ...
        if self.moves_in_turn == []:
            ...
            self.moves_in_turn = self.best_move_depth_alphabeta_timeLimit_all_turn(board,players_pieces_cpy,current_player_idx,self.prob,self.run_time)
        if self.moves_in_turn == [] or self.moves_in_turn is None:  # could not find, prob was too much strict, run on depth 1 only.
            self.moves_in_turn = self.best_move_depth_alphabeta_timeLimit_all_turn(board, players_pieces_cpy_again,
                                                                                   ...
                                                                                   1)
        res = (self.moves_in_turn.pop(0), self.moves_in_turn.pop(0), self.moves_in_turn.pop(0), self.moves_in_turn.pop(0))
        if no_need_2_calc:
            time.sleep(1)
        return res
