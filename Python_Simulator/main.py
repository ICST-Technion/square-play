...
def remoteMain():
    ...
    while True:
        try:
            ...
            if action_code[0] == 2 and game_started:  # regular move
                bytes_received = c.recv(4000)
                array_received = np.frombuffer(bytes_received, dtype=np.intc)
                print("Move:")
                print(array_received)
            ...
            if action_code[0] == 3:  # ai player turn
                if remote_game.players[remote_game.curr_player_num - 1].ai_player:
                    ...
                    if game_started:
                        ...
                        to_send = [float(remote_game.start_game(best_move[0], best_move[1], ))]
                        if to_send != [-1]:
                            game_started = True
            ...
            if action_code[0] == 4 and game_started:  # pass turn
                bytes_received = c.recv(4000)
                array_received = np.frombuffer(bytes_received, dtype=np.intc)
                player_num = array_received[0]
                to_send = [remote_game.pass_turn(player_num)]

            if action_code[0] == -1:
                print("Bye!")
                exit()

            to_send.append(remote_game.last_new_squares)
